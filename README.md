...
- For production, it is recommended to provide your own TON node (with lite server). As a fallback, the service can also connect to a pool of public liteservers via `LITESERVERS_CONFIG_URL`. If you want to use an untrusted node (such as a rented node), you need to set `PROOF_CHECK_ENABLED=true` and specify `NETWORK_CONFIG_URL`.
...
|------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `LITESERVERS_CONFIG_URL`     | optional URL of TON global config to load multiple liteservers at startup, example: `https://ton.org/global.config.json` or `https://ton.org/testnet-global.config.json`. If set, the service uses the liteserver pool from this config instead of a single `LITESERVER` / `LITESERVER_KEY`.                                                                              |
| `LITESERVER`                 | IP and port of a single lite server, example: `185.86.76.183:5815`. Required only if `LITESERVERS_CONFIG_URL` is not set.                                                                                                                                                                                                                                                     |
| `LITESERVER_KEY`             | public key of lite server `5v2dHtSclsGsZVbNVwTj4hQDso5xvQjzL/yPEHJevHk=`. <br/>Be careful with base64 encoding and ENV var. Use ''. Required only if `LITESERVERS_CONFIG_URL` is not set.                                                                                                                                                                                     |
| `LITESERVER_RATE_LIMIT`      | client-side request limit for TON lite client traffic. If you use a rented node with an RPS limit, set it slightly below the node limit. When `LITESERVERS_CONFIG_URL` is used, this limit applies to the whole client pool. Default: 100.                                                                                                                                |
...
| `NETWORK_CONFIG_URL`         | the path to load the network configuration to get the trusted key block from it. This is necessary for proof verification, example: `https://ton.org/global.config.json`. If omitted and `LITESERVERS_CONFIG_URL` is set, the same URL is reused automatically.                                                                                                            |
| `ALERTS_ENABLED`             | `true` - service publishes operational alerts (e.g. withdrawal stuck on underfunded hot wallet) to the shared platform Redis stream consumed by the platform Telegram bot. Default: `false`.                                                                                                                                                                                  |
| `ALERTS_REDIS_URL`           | address of the shared platform Redis, example `redis:6379`. If not set, alerts are disabled.                                                                                                                                                                                                                                                                                  |
| `ALERTS_REDIS_PASSWORD`      | password for the shared platform Redis. If not set then not used.                                                                                                                                                                                                                                                                                                             |
| `ALERTS_REDIS_DB`            | Redis DB number reserved for the alerts stream. Default: `15`.                                                                                                                                                                                                                                                                                                                |
| `ALERTS_PROJECT`             | project slug the platform bot routes alerts on (its Telegram topic). Default: `durak`.                                                                                                                                                                                                                                                                                        |
| `ALERTS_STREAM`              | name of the Redis stream for alerts. Default: `alerts`.                                                                                                                                                                                                                                                                                                                       |
